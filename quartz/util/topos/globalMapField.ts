...
export function createGlobalMapField(
  ...
  function step(dt: number) {
    ...
    for (let sub = 0; sub < count; sub++) {
      ...
      for (const link of links) {
        ...
        const x = (dx / distance) * force,
          y = (dy / distance) * force
        const a = indices.get(link.source.id)!,
          b = indices.get(link.target.id)!
        fx[a] += x
        fy[a] += y
        fx[b] -= x
        fy[b] -= y
      ...
      nodes.forEach((node, i) => {
        ...
        node.vx = (node.vx + (fx[i] / node.mass) * h) * Math.exp(-9 * response * h)
        node.vy = (node.vy + (fy[i] / node.mass) * h) * Math.exp(-9 * response * h)
        node.x += node.vx * h
        node.y += node.vy * h
      })
      collide()
    ...
    nodes.forEach((node, i) => {
      const observedSpeed =
        Math.hypot(node.x - before[i].x, node.y - before[i].y) / Math.max(0.0001, elapsed)
    ...
    const moving = nodes.some(
      (node, i) =>
        Math.hypot(node.x - before[i].x, node.y - before[i].y) > quietDisplacement ||
  ...
  return {
    ...
    snapshot(): MapFieldSnapshot {
      return { version: 1, nodes: nodes.map((node) => ({ ...node })), settled: !active }
    },
