...
当前已恢复地图与性能改造。首页和 `topos.html` 直接进入知识地图：左侧勾选主题，点击分区进入，或用搜索找到对象；“聚焦当前联系”查看邻域，“联系与出处”读方向、原文依据与范围外入口。手机通过同一按钮完整展开关联说明。名称目录是辅助入口，正文仍来自原来的笔记。

打开文章后，点击“收拢”返回原地图；浏览器后退、前进和刷新保留对象、镜头与阅读位置。主题拥有稳定位置，新加入的节点避开已有空间。圆点、名称与关系各自保持可辨认，空间距离不代表数学依赖。

界面更新继续双击 `同步到GitHub.cmd`，预览后输入 `SYNC` 才会上传。这个入口不扫描私人库。写完新笔记后的只读导出仍单独使用 `发布博客.cmd`。本轮实测入口是 `node scripts/verify-map-reading.mjs` 和 `node scripts/verify-map-workflow.mjs`，真实结果见 [验证记录](docs/validation.md)。

以下保留先前接口和暂停点的历史记录；当前状态以上面的恢复说明为准。

暂停点（2026-09-10）：按用户要求暂停后续图谱扩建和材料扩充，保留当前可运行网站及接口。现有 `ReaderCatalog`、`KnowledgeIndex`、Topos 数据载入、正式正文懒加载、稳定对象地址和浏览器历史接口继续使用；`field` 与 `globalMapField` 的节点目标、受力、拖动、快照和恢复保持独立。恢复工作时先核对本节及 `docs/validation.md`，不要把此暂停点当成整个数学图谱目标已完成。`同步到GitHub.cmd` 仍只同步网站工程与现有批准快照，`发布博客.cmd` 保留单独的笔记导出流程。

节点与文字分别绘制：拖动经过其他名称时，节点仍保留可见的圆点、键盘入口与实际连线。节点有各自的布局目标和有界受力参数，真实联系产生局部牵引，接触产生排斥，释放后短暂收敛并停稳。手动意图、过滤与返回位置保留；碰撞让位不代表新增数学关系。选择节点可查看现有关系的具体说明和原文依据。专项验证入口为 `pnpm --package=npm@10.9.2 dlx npm run test:node-space`。
...
原位关系视图保留独立圆点与实际连线；名称没有安全位置时可以退隐，完整名称仍可通过节点焦点和关系说明读取。鼠标悬停不移动场景；拖过的节点留下，直接邻居作有限响应，发生物理接触时也会互相让位。文字保留可用位置，松手后进行一次避让，稳定后停止计算。帮助菜单的“重新整理当前布局”可清除手动落点；新的占位、拖动与返回规则用 `test:node-space` 验证，实际证据及限制记录在 [验证记录](docs/validation.md)。
