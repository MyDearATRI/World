...
动画响应采用按帧合并输入、复用图结构与绘制缓冲区，并让标题逐帧跟随节点。拖动、缩放和主题筛选不再反复重算不变的分组；正文宽高与字体大小不叠加长时间的布局过渡。可用 `pnpm --package=npm@10.9.2 dlx npm run test:motion -- --output artifacts/motion/current` 复跑本机测量；实际前后对照及限制记录在 [验证记录](docs/validation.md)，不把无头浏览器数据当成实体设备帧率。
