...
这次新增了 `同步到GitHub.cmd`：修改网站界面或交互后，双击它即可检查、预览并同步网站工程。看过预览后输入 `SYNC`，它才会普通提交并 push，等待对应提交的 Pages 结果。它沿用已经公开的内容快照，不扫描私人 Obsidian 库；未登记的新文件不会自动加入。检查失败、已有无关暂存文件、分支分叉或预览后文件改变时会停止。推送失败可重开入口重试同一个提交。
